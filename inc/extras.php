<?php
     ...
     * @return html
     */
    if ( ! function_exists( 'refru_post_navigation' ) ) {
        function refru_post_navigation() {

            $post_nav_bck = '';
            $post_nav_bck_next = '';
            $prev_post = get_previous_post();
            if ( ! empty( $prev_post ) ):
                $portfolio_image = wp_get_attachment_image_src( get_post_thumbnail_id( $prev_post->ID ), 'refru_portfolio' );
            ...
            if ( ! empty( $next_post ) ):
                $portfolio_image = wp_get_attachment_image_src( get_post_thumbnail_id( $next_post->ID ), 'refru_portfolio' );
                if ( ! empty( $portfolio_image ) ) {
...
<nav class="navigation post-navigation">
    <div class="nav-links">
        ...
        <div class="nav-previous" <?php echo wp_kses_post( $post_nav_bck ); ?>>
            <?php
                $prev_text = esc_html__( 'Previous Post', 'refru' );
                    ?>
            <a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>"
                rel="prev"><span><?php echo wp_kses_post( $prev_text ); ?></span><?php echo esc_html( $prev_post->post_title ); ?></a>
        </div>
        <?php endif; ?>
        <?php if ( ! empty( $next_post ) ): ?>
        <div class="nav-next" <?php echo wp_kses_post( $post_nav_bck_next ); ?>>
            <?php
                $next_text = esc_html__( 'Next Post', 'refru' );
                    ?>
            <a href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>"
                rel="next"><span><?php echo wp_kses_post( $next_text ); ?></span><?php echo esc_html( $next_post->post_title ); ?></a>
        </div>
        <?php endif; ?>
    </div>
</nav>
<?php endif;
     ...
     */
    function refru_get_theme_mod( $option, $default ) {
        $get_arg = str_replace( 'refru_', '', $option );
        $refru_option = get_theme_mod( $option, $default );
        if ( isset( $_GET[$get_arg] ) ) {
            $refru_option = sanitize_text_field( wp_unslash( $_GET[$get_arg] ) );
        }
        return $refru_option;
    }
     ...
     *
     */
    function refru_custom_upload_mimes( $existing_mimes = array() ) {
        // Add the file extension to the array
        $existing_mimes['svg'] = 'image/svg+xml';
        return $existing_mimes;
    }
    add_filter( 'upload_mimes', 'refru_custom_upload_mimes' );
